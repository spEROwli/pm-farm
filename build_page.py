...
MAX_AGE_DAYS = 3  # keep in sync with pmfarm.py
...
_ROLE_TYPE_MAP = [
    ("apm", ["associate product manager", "associate product"]),
    ("pm",  ["product manager"]),
]
...
_SENTENCE_YR = re.compile(r'\b(\d+)\s*\+?\s*(?:years?|yrs?)\b')


def _in_range(row: dict) -> bool:
    soft = _years_is_soft(row)
    # Primary: use scraped years_raw field
    n = _years_num(row.get("years_raw", ""))
    if n is not None and n > 3 and not soft:
        return False
    # Sanity check: re-parse years_sentence so parser bugs can't sneak 5+ yr roles through
    sentence = (row.get("years_sentence") or "").lower()
    if sentence and sentence != "not stated":
        nums = [int(m) for m in _SENTENCE_YR.findall(sentence)]
        if nums and max(nums) > 3 and not soft:
            return False
    return True


_ENTRY_KEYWORDS = ["associate product", "associate pm", " apm", "rotational product", "new grad"]
_SENIOR_KEYWORDS = ["senior ", "sr. ", "lead ", "group product", "staff product"]


def _seniority_rank(row: dict) -> int:
    """0 = entry/associate, 1 = plain PM, 2 = senior/lead. Used to sort within tier."""
    t = row.get("title", "").lower()
    if any(kw in t for kw in _ENTRY_KEYWORDS):
        return 0
    if any(kw in t for kw in _SENIOR_KEYWORDS):
        return 2
    return 1


def _is_priority(row: dict) -> bool:
    # Entry-level and associate roles always surface first regardless of sector
    if _seniority_rank(row) == 0:
        return True
...
def _render_bucket(rows: list) -> str:
    ...
    geo_counts: dict = {}
    for r in rows:
        g = _geo_rank(r.get("loc_class", "unknown"))
    ...
    for r in rows:
        ...
        if geo != last_geo:
            sec_id += 1
            label = _GEO_LABEL.get(geo, "Other")
            count = geo_counts.get(geo, 0)
            parts.append(f'  <div class="sec" data-sec="{sec_id}" data-tier="{geo}">{label} <span class="sec-count">({count})</span></div>')
            last_geo = geo
...
_DESC_TAIL = "· filtered for NYC + SF entry-to-mid-level positions"


def _page_desc(rows: list) -> str:
    """Name only the sources that actually produced these roles.

    A hardcoded list goes stale silently: a missing Bright Data key drops three
    ...
    if not names:
        return f"PM roles scraped daily from live hiring APIs {_DESC_TAIL}"
    joined = names[0] if len(names) == 1 else ", ".join(names[:-1]) + " &amp; " + names[-1]
    return f"PM roles scraped daily from {joined} {_DESC_TAIL}"
...
def build():
    ...
    # Stretch bucket only renders when it has roles — an empty dropdown reads as broken.
    stretch_block = ""
    if bucket_b:
        stretch_block = f"""  <details>
    <summary>Stretch bucket — 4+ yrs stated ({len(bucket_b)} roles)</summary>
    <div id="blist">
{_render_bucket(bucket_b)}
    </div>
  </details>"""
...
<style>
  ...
  * {{ box-sizing:border-box; margin:0; padding:0; -webkit-tap-highlight-color:transparent; }}
  ...
  header {{ position:sticky; top:0; z-index:20; background:var(--header-bg);
           ...
           border-bottom:.5px solid var(--line); padding:18px 18px 11px; }}
  ...
  h1 {{ font-family:var(--serif); font-size:28px; font-weight:500; letter-spacing:-.018em; color:var(--fg1); }}
  h1::after {{ content:"."; color:var(--rust); }}
  .subtitle {{ font-family:var(--mono); font-size:11px; color:var(--fg3); white-space:nowrap; }}
  .hdesc {{ font-family:var(--serif); font-style:italic; font-size:14.5px; line-height:1.5;
           color:var(--fg2); margin-top:9px; max-width:60ch; }}
  ...
  /* ── Cards ── */
  .card {{ background:var(--card); border:.5px solid var(--line); border-top:3px solid transparent;
          border-radius:12px; padding:18px 20px; margin-bottom:12px; display:flex; flex-direction:column; gap:8px;
  ...
  .card.pri {{ border-top-color:var(--rust); }}
  .card.done {{ opacity:.5; }}
  .top {{ display:flex; align-items:center; justify-content:space-between; gap:10px; }}
  .co {{ font-family:var(--mono); font-size:12px; letter-spacing:.04em; color:var(--fg3); white-space:nowrap; }}
  .ti {{ font-family:var(--sans); font-size:16px; font-weight:700; line-height:1.3; letter-spacing:-.012em; color:var(--fg1); }}
  ...
  .sec-count {{ font-weight:400; opacity:.55; }}
  .sec {{ display:flex; align-items:center; gap:9px; font-family:var(--sans); font-size:11px; font-weight:700;
  ...
  .btn {{ display:block; text-align:center; background:transparent; color:var(--fg1);
         border:1.5px solid var(--fg1); font-family:var(--sans); font-size:13px; font-weight:600; padding:9px; border-radius:6px;
         text-decoration:none; margin-top:4px; transition:background .15s,color .15s,border-color .15s; }}
  .btn:hover {{ background:var(--fg1); color:var(--bg); }}
  ...
  .foot {{ font-family:var(--mono); font-size:10.5px; color:var(--fg3);
          padding:28px 0 48px; border-top:.5px solid var(--line); margin-top:28px; text-align:center; }}
  .foot a {{ color:var(--fg3); text-decoration:none; }}
  .foot a:hover {{ color:var(--fg1); }}
</style></head><body>
...
{stretch_block}
  ...
  <footer class="foot">
    <a href="https://github.com/sperowli/pm-farm" target="_blank" rel="noopener">github.com/sperowli/pm-farm</a>
  </footer>
</main>
